...
class PDFPipelineEvaluator:
    ...
    def setup_pipeline(self):
        ...
        logger.info("=" * 60)
        logger.info("PDF PIPELINE EVALUATION")
        logger.info("=" * 60)
        ...
        self.embeddings = self.embedder.encode(self.texts, show_progress=True)
        embed_time = (time.time() - t0) * 1000
        logger.info(f"[3] Embedding: {embed_time:.0f}ms ({len(self.embeddings)} vectors)")
        ...
        logger.info(f"[6] SQLite Save: {sqlite_time:.0f}ms")
    ...
    def _calculate_groundedness(self, answer: str, context: str) -> float:
        ...
        # Simple heuristic: sentences in answer that have support in context
...
def main():
    ...
    evaluator = PDFPipelineEvaluator(pdf_path)
    ...
    if results:
        ...
        logger.info(f"  Total Eval:    {total_latency:.0f}ms ({len(results)} queries)")
